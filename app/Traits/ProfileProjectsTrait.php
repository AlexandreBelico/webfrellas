    ...
    public function showProfileProjects(Request $request, $user_id)
    {
        ...
        if (isset($user) && count($user->profileProjects)):
            ...
            foreach ($user->profileProjects as $project):
                ...
                $image = ImgUploader::get_image("public/project_images/thumb/$project->image");
                $html .= '<!--Project Start-->
        ...
        if ($request->hasFile('image')) {
            ...
            if (is_array($image)) {
                foreach ($image as $img) {
                    $fileName = ImgUploader::UploadImage('public/temp_project_images', $img);
                    $this->putImageInSession($fileName);
                }
            } else {
                $fileName = ImgUploader::UploadImage('public/temp_project_images', $image);
                $this->putImageInSession($fileName);
            }
            echo $fileName;
        } else {
