...
trait JobTrait
{
    ...
    private function assignJobValues($job, $request)
    {
        ...
        $job->job_rate_type = $request->input('job_rate_type');
        return $job;
    }
    ...
    public function storeFrontJob(JobFrontFormRequest $request)
    {
        ...
        try {
            ...
            $request->merge(['expiry_date' => date('Y-m-d h:m:s', strtotime('+10 years'))]);
            $request->merge(['salary_currency' => 'USD']);

            //$request->expiry_date = date('Y-m-d h:m:s', strtotime('+1 month'));
            //dd($request);
            //DB::enableQueryLog();
            $job = $this->assignJobValues($job, $request);
            ...
            $content = "<a href='" . url('job/' . $job->slug) . "' target='_blank'> $job->title</a>";
...
//            ];
//
//            $sendNotification = Notification::create($notification);
            ...
            $pusher->trigger('notification', 'notification-event', $jobNotification);
    ...
    {
        $request->request->add(['salary_currency' => 'USD']);
        $job = Job::findOrFail($id);
        $job = $this->assignJobValues($job, $request);
        /*         * ******************************* */
