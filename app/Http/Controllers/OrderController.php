...
class OrderController extends Controller
{
    ...
    public function weeklyPayment() {
        ...
        if(!empty($completedMileStones->toArray())){
            foreach ($completedMileStones as $payDetails) {
                $amount = $payDetails->price;
                $milestoneTitle = $payDetails->milestone_title;
                $milestoneDescription = $payDetails->description;
                $milestonePrice = $payDetails->price;
                $milestoneJobId = $payDetails->job_id;

                /* Set a new payer for each transaction */
                $payer = new Payer();
                $payer->setPaymentMethod('paypal');
        
                $item_1 = new Item();
                $item_1->setName($milestoneTitle) /** item name **/
                ...
                $payee = new Payee();
                $payee->setEmail("cdo-facilitator@logixbuilt.com");

                $transaction = new Transaction();
                $transaction->setAmount($amount)
                            ->setPayee($payee)
                            ->setItemList($item_list)
                            ->setDescription('payment for milestone '.$milestoneTitle);
                            
                $redirect_urls = new RedirectUrls();
                        $redirect_urls->setReturnUrl(URL::route('payment.approvallink', $milestoneJobId))
                            ->setCancelUrl(URL::route('payment.approvallink', $milestoneJobId));
                ...
                // print_r($approvalUrl = $payment->getApprovalLink()); echo "<br>";
    ...
    public function approvalLink($jobId) {
    ...
    {   
        ...
        /* Condition for validation fails */
        if($validation->fails())
        {
            ...
            if(Auth::guard('company')->check()){
                ...
                if($employer_details)
                {
                    ...
                    if($pay_type == 'credit_card')
                    {
                        ...
                        /* set credit card and details from the form */
                        $card = new creditCard();
                        $card->setType($card_type)
                            ->setNumber($request->card_no)
                            ->setExpireMonth($request->ccExpiryMonth)
                    ...
                    /* set items for transactions right now for single if multiple we can create multiple items like Amazon  cart having multiple items and its total */
                    $item1 = new item();
                    $item1->setName($description)
                            ->setDescription($description)
                            ->setCurrency('USD')
                    ...
                    /* set transaction with all Items amd its total */
                    $transaction = new transaction();
                    $transaction->setAmount($amount)
                        ->setItemList($itemList)
                        ->setDescription($description)
                    ...
                    /* Define variables */
                    if($payment->state ==='approved'){
                        /* Get transaction details from payment */
                        $transactions = $payment->getTransactions();
                        $relatedResources = $transactions[0]->getRelatedResources();
                        $sale = $relatedResources[0]->getSale();
                        $saleId = $sale->getId();
                        ...
                        $package_id = $request->input('job_id');
                        ...
                        /* set data for insert in payment table */
                        $payment_data = [];
                        $payment_data['employee_id']=Auth::guard('company')->user()->id;
                        $payment_data['candidate_id']=$hired_user;
                        $payment_data['job_apply_id']=$vals[0]->id;
                        $payment_data['job_id']=$package->id;
                        $payment_data['sale_id']=$saleId;
                        $payment_data['payment_status']=$payment->state;
                        $payment_data['transaction_details']=json_encode($relatedResources);
                        $payment_data['invoice_number']=$invoice_number;
                        ...
                        if($payment_details){
                            ...
                            /* Collect all details for milestone to insert in milestone table */
                            ...
                            /* Assign values to object for insert in database for Milestone */
                            ...
                            /* Insert data in Milestone table */
                            if($milestones->save()){
                               $milestoneId = $milestones->id;

                              /* Update inserted Milestone id in payment table */
                              PaymentDetails::where('id', $paymentId)
                            ...
                            /* Redirect to Milestone listing page */
