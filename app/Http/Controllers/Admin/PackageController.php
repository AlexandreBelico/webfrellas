...
class PackageController extends Controller
{
    ...
    public function profitSet()
    {
        ...
        return view('admin.package.profit')
    ...
    public function storeProfit(Request $request)
    {
        $this->validate($request, [
            ...
            'profit.required' => 'Percentage is required.',
        ]);
        if($request->input('pid')){
            $percent = Percentage::findOrFail($request->input('pid'));
            $percent->percent = $request->input('profit');
            $percent->update();
        }else{
        ...
        flash('Profit has been updated!')->success();
        return \Redirect::route('profit.package');
    }
