...
class DataArrayHelper
{
    ...
    public static function TotalMessagesCountEmp($cmp)
    {
        $messages = CompanyMessage::where('company_id', $cmp)->get();
        $ids = array();
        ...
        foreach ($job_applications as $userIds) {
            ...
            $unread = DB::select('select count(is_read) as unread from messages where messages.to = '. $company_id . ' and messages.from = '. $userId .' and is_read = 0 and job_id!=null');
                $unread = $unread[0]->unread;
                
                $userDetails[0]->unread = $unread;
                $total_msg += $unread;
        ...
        $total_unread_message = Message::where('to', Auth::guard('company')->user()->id)->where('is_read',0)->count();
    ...
    public static function countmessageCandidates($canID)
    {
        $messages = CompanyMessage::where('seeker_id',$canID)->get();
        $ids = array();
        foreach ($messages as $key => $value) {
        ...
        if(!empty($allcompanies)){
            foreach ($allcompanies as $companyId) {
                ...
                $unread = DB::select('select count(is_read) as unread from messages where messages.to =  '. Auth::id() . ' and messages.from = '. $companyId .' and is_read = 0 and job_id!=null');
                $unread = $unread[0]->unread; 

                $singleCompany[0]->unread = $unread;
                $total_msg_candidates += $unread;
